	...
	for (auto& classPair : m_structs)
	{
		for (UnrealObject& unrealObj : classPair.second)
		{
			if (unrealObj.FullName == structFullName)
			{
				...
				if (scriptStruct->PropertiesSize >= propertySize)
				{
					largestStruct = unrealObj;
					propertySize = scriptStruct->PropertiesSize;
				}
			}
		}
	}
...
void GLogger::LogClassSize(class UClass* uClass, size_t localSize)
{
	...
	if (m_file.is_open() && uClass)
	{
		...
		m_file << "Error: Actual size " << uClass->PropertiesSize << std::endl;
	}
#endif
}
...
void GLogger::LogStructPadding(class UScriptStruct* uScriptStruct, size_t padding)
{
	...
	if (m_file.is_open() && uScriptStruct)
	{
		...
		m_file << "Info: Property size " << Printer::Hex(uScriptStruct->PropertiesSize, 1) << "\n";
	...
	void GenerateStruct(std::ofstream& file, const UnrealObject& unrealObj)
	{
		if (unrealObj.IsValid())
		{
			...
			if (!GConfig::IsTypeOveridden(structNameCPP))
			{
				...
				{
					size = (scriptStruct->PropertiesSize - superField->PropertiesSize);
					lastOffset = superField->PropertiesSize;
					...
					structStream << " (" << Printer::Hex(superField->PropertiesSize, EWidthTypes::Size);
					structStream << " - " << Printer::Hex(scriptStruct->PropertiesSize, EWidthTypes::Size);
					structStream << ")\n";
				...
				{
					size = scriptStruct->PropertiesSize;
					structStream << "// " << Printer::Hex(size, EWidthTypes::Size) << "\n";
				...
				if (lastOffset < scriptStruct->PropertiesSize)
				{
					missedOffset = (scriptStruct->PropertiesSize - lastOffset);
				...
				if (scriptStruct->MinAlignment)
				{
					...
					for (int32_t i = 0; actualSize < scriptStruct->PropertiesSize; i++)
					{
						actualSize += scriptStruct->MinAlignment;
					}

					if ((lastOffset < actualSize) && (actualSize > scriptStruct->PropertiesSize))
					{
...
namespace ClassGenerator
{
	...
	void GenerateClassMembers(std::ostringstream& classStream, class UClass* uClass, EClassTypes classType)
	{
		if (uClass && (classType != EClassTypes::Unknown))
		{
			...
			if (!members.empty())
			{
				if (uClass->PropertiesSize == localSize)
				{
					lastOffset = startOffset;
					uint32_t unknownDataIndex = 0;
					...
					if (lastOffset < uClass->PropertiesSize)
					{
						missedOffset = (uClass->PropertiesSize - lastOffset);
			...
			if (!GConfig::IsTypeOveridden(classNameCPP))
			{
				...
				for (UProperty* uProperty = static_cast<UProperty*>(uClass->Children); uProperty; uProperty = static_cast<UProperty*>(uProperty->Next))
				{
					if (uProperty
						...
						&& (!uSuperClass || (uSuperClass && (uSuperClass != uClass) && (uProperty->Offset >= uSuperClass->PropertiesSize))))
					{
						UnrealProperty unrealProp(uProperty);
				...
				{
					size = (uClass->PropertiesSize - uSuperClass->PropertiesSize);
					lastOffset = uSuperClass->PropertiesSize;

					classStream << "// " << Printer::Hex(size, EWidthTypes::Size);
					classStream << " (" << Printer::Hex(uSuperClass->PropertiesSize, EWidthTypes::Size);
					classStream << " - " << Printer::Hex(uClass->PropertiesSize, EWidthTypes::Size);
					classStream << ")\n" << "class " << classNameCPP << " : public " << UnrealObject::CreateValidName(uSuperClass->GetNameCPP());
				}
				else
				{
					size = uClass->PropertiesSize;
...
namespace Generator
{
	...
	void GenerateHeaders()
	{
		...
		Printer::Header(headersFile, "SdkHeaders", "hpp");
	...
	void GenerateDefines()
	{
		...
		Printer::Header(definesFile, "GameDefines", "hpp");
		...
		if (std::filesystem::exists(directory))
		{
			...
			for (const UnrealObject& packageObj : *packages)
			{
				if (packageObj.IsValid())
				{
					...
					Printer::Header(file, (packageObj.ValidName + "_structs"), "hpp");
					...
					Printer::Header(file, (packageObj.ValidName + "_classes"), "hpp");
					...
					Printer::Header(file, (packageObj.ValidName + "_parameters"), "hpp");
					...
					Printer::Header(file, (packageObj.ValidName + "_classes"), "cpp");
	...
	bool Initialize(bool bCreateLog)
	{
		...
		if (!AreGlobalsValid())
		{
			...
			if (AreGlobalsValid())
			{
				...
				UStruct::Register_PropertiesSize();
